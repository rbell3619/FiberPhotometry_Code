function  graph_Octave_HitvFA(HitMatrix_GRAB,FA_Matrix_GRAB,Epoc_Range,ISO_fs,ITI,loc,type,RT_Hits,N,Animal_ID,tone)
figure
% m=tiledlayout(1,7);
% m.TileIndexing='columnmajor';
% m.TileSpacing="tight";
if length(RT_Hits)==1
    RT_Hits=Octave_Merge(RT_Hits);
end
for a=1:4
if length(RT_Hits)>1
    RT_Hits_Pull=RT_Hits{a};
end
Grab_stream_Hit=HitMatrix_GRAB{a};
Grab_stream_FA=FA_Matrix_GRAB{a};
...
[Master_Matrix_FA,zerror_FA]=zscore_fp(GRAB_avg_FA,ITI,ts2);
    ...
    line([mean(RT_Hits_Pull)+.5 mean(RT_Hits_Pull)+.5],[-20 ,20], 'Color', 'g', 'LineWidth', 2)
    ...
    title(["Response Around Tone Onset",loc+tone{a}+type,Animal_ID],'FontSize',25)
end
