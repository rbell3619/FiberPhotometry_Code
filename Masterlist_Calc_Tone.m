function [AUC,Amp]=Masterlist_Calc_Tone(Master_Call_Matrix,N,Epoc_Range,ISO_fs,ITI,Masterlist_ID,type,Location,RT)
...
for b=1:7
    Grab_stream=Grab{b};
   if isempty(Grab_stream)==1
Response_AUC(a,b)=NaN;
Reward_AUC(a,b)=NaN;
Max_amp(a,b)=NaN;
min_amp(a,b)=NaN;
   else
...
for a= 1:length(Masterlist_ID)
    for b=1:7
    Response_AUC_Sub(b,:)=[Masterlist_ID(a),Location,type,tone(b),AUC_type(1),Response_AUC(a,b)];
    Reward_AUC_Sub(b,:)=[Masterlist_ID(a),Location,type,tone(b),AUC_type(2),Reward_AUC(a,b)];

    Amp_Max_Sub(b,:)=[Masterlist_ID(a),Location,type,tone(b),Amp_type(1),Max_amp(a,b)];
    Amp_Min_Sub(b,:)=[Masterlist_ID(a),Location,type,tone(b),Amp_type(2),min_amp(a,b)];
    end 
